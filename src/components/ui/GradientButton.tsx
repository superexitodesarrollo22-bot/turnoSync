        ...
        return (
            <AnimatedPressable onPress={onPress} disabled={disabled || loading} scaleValue={0.97} style={style}>
                <View style={[
                    ...
                    {renderContent(colors.accent)}
                </View>
            </AnimatedPressable>
        );
    }

    const gradientColors: readonly [string, string, ...string[]] = [(colors as any).accent, (colors as any).accentLight || (colors as any).accent];
    const textColor = '#0D0D1A'; // Texto dark para el primary
